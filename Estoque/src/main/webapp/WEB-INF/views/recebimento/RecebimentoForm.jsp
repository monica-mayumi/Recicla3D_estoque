...
<body>
	...
	<div class="container">
		<br>
		<f:form action="${s:mvcUrl('salvarRecebimentoUrl').build()}"
				...
				<div class="col s12">
					...
					<div class="card">
						<div class="card-content">
							...
							<span class="card-title center-aling">Itens</span>
							<div class="row">
								<div class="input-field col s12">
									<div id="dadosTabela">
										<table class="responsive-table">
											...
											<tbody>
												<c:forEach items="${recbimentoForm.itens }" var="itemReceb"
													varStatus="status">
													<tr>
														...
														<td>
															<div class="input field">
																<f:input path="itens[${status.index}].quantidade"
																	type="number" step="1" />
																<f:errors path="itens[${status.index}].quantidade"
																	cssClass="helper-text" />
															</div>
														</td>
														<td>
															<div class="input field">
																<f:input path="itens[${status.index}].precoUnitario"
																	type="number" step="0.01" />
																<f:errors path="itens[${status.index}].precoUnitario"
																	cssClass="helper-text" />
															</div>
														</td>
														<td class="right-align vlrTotal"><fmt:formatNumber
																value="${itemReceb.valorTotal}" type="currency" /></td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
					...
					<div class="card-action">
						<input class="btn-small blue darken-3" type="submit"
							value="Confirmar"> <a
							href="${s:mvcUrl('listarRecebimentoUrl').build()}"
	...
	function atualizaTabelaItens(dadosRetorno) {
			...
			//Funcao executada quando ocorrer um erro
			...
			var elQuantidadde = elementoLinha("input[name$='quantidade']")
			var elPrecoUnitario = elementoLinha("input[name$='precoUnitario']")
			var elValorTotal = elementoLinha("input[name$='valorTotal']")
			var valorCalculado = elQuantidade.val()*elPrecoUnitario.val();
			 ...
			 //buscar os itens e carregar a tabela 
