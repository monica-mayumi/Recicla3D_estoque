...
<body onload="myFunction('limpa')">
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="input-field col s3 offset-s3">
				<a
					class="btn-floating btn-large waves-effect waves-light cyan darken-4 right"
					title="novo" href="${s:mvcUrl('novoRecebimentoUrl').build()}">
		...
		<div class="row">
			<div class="responsive-table col s12">
				<table>
					<thead>
						<tr>
							<th class="center-align">Série NF</th>
							<th class="center-align">Número NF</th>
							<th class="center-align">Data NF</th>
							<th class="center-align">Nº Pedido</th>
							<th class="center-align">Fornecedor</th>
							<th class="center-align">Recebimento</th>
					...
					<tbody>
						<c:forEach items="${listaPaginada.content}" var="registro">
							<tr>
								<fmt:parseDate value="${registro.dataNotaFiscal}"
									pattern="yyyy-MM-dd" var="dtaNotaFiscal" type="date" />
								<fmt:parseDate value="${registro.dataRecebimento}"
									pattern="yyyy-MM-dd" var="dtaRecebimento" type="date" />

								<td class="center-align">${registro.serieNotaFiscal}</td>
								<td class="center-align">${registro.numeroNotaFiscal}</td>
								<td class="center-align"><fmt:formatDate value="${dtaNotaFiscal}"
										pattern="dd/MM/yyyy" /></td>
								<td class="center-align">${registro.pedido.id}</td>
								<td class="center-align">${registro.pedido.fornecedor.razaoSocial}</td>
								<td class="center-align"><fmt:formatDate value="${dtaRecebimento}"
										pattern="dd/MM/yyyy" /></td>
								<td class="center-align"><a
									class="btn-small indigo darken-2" title="detalhes"
									href="${s:mvcUrl('detalharRecebimentoUrl').arg(0, registro.id).build()}"><i
										class="material-icons">more_horiz</i></a>
									

									<button class="btn-small pink accent-4 modal-estornar"
										title="estornar" type="button" data-target="modalEstornar"
										data-descr="Nota Fiscal Nº ${registro.numeroNotaFiscal}">
										<i class="material-icons">cached</i>
										<f:form
											action="${s:mvcUrl('estornoRecebimentoUrl').arg(0, registro.id).build()}"
											method="post">
										</f:form>
									</button>
									</td>
					...
					<ul class="pagination">
						<li class="${listaPaginada.hasPrevious() ? '' : 'disabled'}">
							<c:if test="${listaPaginada.hasPrevious()}">
								<a href="#" class="paginacao"
									data-pagina="${listaPaginada.getNumber()}"><i
						...
						<li class="${listaPaginada.hasNext() ? '' : 'disabled'}"><c:if
								...
								<a href="#" class="paginacao"
									data-pagina="${listaPaginada.getNumber()+2}" rel="next"><i
									class="material-icons">chevron_right</i></a>
							</c:if> <c:if test="${!listaPaginada.hasNext()}">
								<a href="#" class="paginacao" rel="next"><i
									class="material-icons">chevron_right</i></a>
							</c:if>
							</li>
					</ul>
	...
	<%@ include file="../base/modalEstorno.jsp"%>
</body>
